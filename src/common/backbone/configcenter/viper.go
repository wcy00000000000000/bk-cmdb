...
func Crypto(prefix string) (*cryptor.Config, error) {
	...
	if !parser.isSet(prefix) {
		return new(cryptor.Config), nil
	}

	conf := &cryptor.Config{
		Algorithm: cryptor.Algorithm(parser.getString(prefix + ".algorithm")),
	}

	switch conf.Algorithm {
	case cryptor.Sm4:
		if !parser.isSet(prefix + ".sm4") {
			return nil, errors.New("sm4 config is not set")
		}
	...
	case cryptor.AesGcm:
		if !parser.isSet(prefix + ".aesGcm") {
			return nil, errors.New("aes-gcm config is not set")
		}
